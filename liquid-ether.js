  ...
  class LiquidEther {
    constructor(container, options = {}) {
      ...
      this.options = {
        ...
        isBounce: false,
        isBounce: false,
        colors: ['#06b6d4', '#f97316', '#67e8f9'], // Code&Craft Theme (Teal, Orange, Cyan)
        autoDemo: true,
        autoSpeed: 0.5,
        autoIntensity: 2.2,
    ...
    init() {
      try {
        if (!this.container) {
          console.error("LiquidEther: No container provided.");
          return;
        }

      // Helper to create palette texture
      ...
      class CommonClass {
        ...
        init(container) {
          ...
          this.renderer.setClearColor(new THREE.Color(0x000000), 0); // Transparent
      ...
      class ShaderPass {
        ...
        init() {
          ...
          if (this.uniforms) {
            ...
            // Fix: React Bits often setsRawShaderMaterial manually. 
            // In vanilla three.js we need to ensure uniforms are proper clones or refs.
            // Using logic as provided.
      ...
      class Output {
        ...
        init() {
          this.simulation = new Simulation(this.simOptions);
          this.scene = new THREE.Scene();
          this.camera = new THREE.Camera();
      ...
      class WebGLManager {
        ...
        init() {
          ...
          this.output = new Output(this.props.simOptions);
        }
        resize() {
          Common.resize();
          this.output.resize();
      ...
      containerStyle.position = containerStyle.position || 'relative';
      containerStyle.overflow = containerStyle.overflow || 'hidden';
      containerStyle.touchAction = 'none'; // from CSS

      this.webgl = new WebGLManager({
        $wrapper: this.container,
        autoDemo: this.options.autoDemo,
        autoSpeed: this.options.autoSpeed,
        autoIntensity: this.options.autoIntensity,
        takeoverDuration: this.options.takeoverDuration,
        autoResumeDelay: this.options.autoResumeDelay,
        autoRampDuration: this.options.autoRampDuration,
        simOptions: this.options // pass main options specifically
      });
      ...
      document.addEventListener('visibilitychange', () => {
        if (document.hidden) {
          if (this.webgl) this.webgl.pause();
        } else if (this.isVisible) {
          if (this.webgl) this.webgl.start();
        }
      });
